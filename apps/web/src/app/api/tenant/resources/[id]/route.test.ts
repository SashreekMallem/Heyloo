...
function patchRequest(body: unknown) {
  return new Request("http://localhost/api/tenant/resources/r1", {
    method: "PATCH",
...
describe("PATCH /api/tenant/resources/[id]", () => {
  ...
  it("422s on an invalid capacity", async () => {
    ...
    const res = await PATCH(patchRequest({ capacity: -1 }), {
      params: Promise.resolve({ id: "r1" }),
    });
  ...
  it("404s when the resource doesn't belong to the caller's tenant", async () => {
    serverQueue = { resources: [{ data: null, error: null }] };
    mockGetUser = async () => ({ data: { user: mockUser } });
    const res = await PATCH(patchRequest({ name: "Renamed" }), {
      params: Promise.resolve({ id: "r1" }),
    });
  ...
  it("updates a resource scoped to the caller's tenant", async () => {
    serverQueue = {
      resources: [{ data: { id: "r1" }, error: null }, { error: null }],
    };
    mockGetUser = async () => ({ data: { user: mockUser } });
    const res = await PATCH(patchRequest({ name: "Renamed" }), {
      params: Promise.resolve({ id: "r1" }),

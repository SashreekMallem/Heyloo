...
describe("importTwilioNumberIntoRetell", () => {
  it("posts /import-phone-number with a single-element inbound_agents array (VERIFY-7)", async () => {
    ...
    expect(capturedBody).toEqual({
      ...
      inbound_agents: [{ agent_id: "agent_1" }],
    });
  });

  it("includes optional SIP trunk auth fields only when provided", async () => {
    let capturedBody: unknown;

    ...
    content: {
      ...
      states: [
        ...
        },
        {
          ...
          prompt_fragment:
            "Thank the caller, let them know an attorney will review the intake (including the conflict check) and follow up, and record the full intake as a message for the firm.\n\nHard guardrail \u2014 true in this state and every other state in this call, with no exceptions: never give legal advice, never offer an opinion on the merits or likely outcome of the caller's case, and never quote a fee beyond the configured consult fee ({{consult_fee_text}}). If pressed, say only that an attorney will review the details and follow up \u2014 never improvise around this rule.",
          allowed_tools: ["take_message"],
          extraction: [
            { field: "legal_advice_given", type: "boolean" },
            {
    ...
    content: {
      ...
      states: [
        ...
        },
        {
          ...
          prompt_fragment:
            "The caller wants a valuation/quote or just isn't ready to schedule a showing yet. Take a message per the structured-lead-capture rule so an agent can follow up.",
          allowed_tools: ["take_message"],
          extraction: [
            {
              field: "classification",
      ...
      states: [
        {
          ...
          name: "Intake",
          prompt_fragment:
            "Collect, one at a time: the caller's name \u00b7 their phone number \u00b7 the reason for the call. If the business can book what they need, offer check_availability/create_booking. Otherwise take a message with a clear callback window and let them know when to expect a call back.",
          allowed_tools: [
            "check_availability",
            "create_booking",
            "join_waitlist",

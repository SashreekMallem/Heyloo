...
describe("dispatchTextTool", () => {
  ...
  describe("verify_phone", () => {
    ...
    it("blocks a distinct 4th phone number in one conversation, but keeps allowing resends to a number already tried (SMS-bombing regression)", async () => {
      const { sql, calls } = makeSql({ "insert into public.messages_outbound": [{ id: "msg-1" }] });
      const conversation = makeConversation({
        channel: "web_chat",
        phoneE164: null,
      ...
      // resending to an already-attempted number is unaffected by the cap

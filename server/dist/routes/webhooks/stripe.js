    ...
    switch (event.type) {
            ...
            if (orderId) {
                ...
                if (order) {
                    ...
                    if (restaurant) {
                        ...
                        sendOrderConfirmationSMS(order.customer_phone, order.id, restaurant.name, order.order_type).catch((err) => {
                        ...
                        if (restaurant.pos_type && restaurant.pos_type !== 'none') {
                            ...
                            sendOrderNotificationSMS(restaurant.manager_phone, order, restaurant.name).catch((err) => {
        ...
        case 'payment_intent.payment_failed': {
            ...
            if (orderId) {
                await supabase
                    ...
                    .eq('id', orderId);
                logger.warn({ orderId, paymentIntentId: paymentIntent.id }, 'Payment failed for order');
            }
            break;
        }
        case 'payment_intent.canceled': {
            ...
            if (orderId) {
                await supabase
                    ...
                    .eq('id', orderId);
                logger.info({ orderId, paymentIntentId: paymentIntent.id }, 'Payment canceled for order');
            }
            break;
        }
        case 'checkout.session.completed': {
            ...
            if (orderId && session.payment_status === 'paid') {
                await attachStripePaymentResult(orderId, session.payment_intent);
                ...
                if (order) {
                    ...
                    if (restaurant) {
                        ...
                        if (restaurant.pos_type && restaurant.pos_type !== 'none') {
                            ...
                            const { sendOrderNotificationSMS } = await import('../../lib/sms.js');
                            sendOrderNotificationSMS(restaurant.manager_phone, order, restaurant.name).catch((err) => {
                                logger.error({ err, orderId }, 'Failed to send order notification to manager after checkout');
                            });
                        }
                    }
                }

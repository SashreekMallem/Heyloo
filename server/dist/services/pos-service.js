import { getPosIntegration } from '../integrations/factory.js';
 ...
 */
export async function syncMenuFromPos(restaurantId, locationId, syncSource = 'manual') {
    ...
    try {
        ...
        if (locationId) {
            ...
            if (locationError || !location) {
                logger.error({ restaurantId, locationId, locationError }, '[POS Sync] Failed to find specified location');
            ...
            logger.info({ restaurantId, locationId: resolvedLocationId, externalId }, '[POS Sync] Syncing specific location');
        }
 ...
 * @param syncSource - 'auto' for automatic syncs (after connection), 'manual' for user-triggered syncs
 */
export async function syncMenuFromPosAsync(restaurantId, locationId, syncSource = 'auto') {
    // Run sync in background - don't await, just fire and forget
    syncMenuFromPos(restaurantId, locationId, syncSource)
        .then((result) => {
        logger.info({ restaurantId, locationId, count: result.count, provider: result.provider, syncSource }, '[POS Sync Async] Menu sync completed successfully');
    })
        .catch((err) => {
        // Log error but don't throw - connection should succeed even if sync fails
        logger.error({ err, restaurantId, locationId, syncSource }, '[POS Sync Async] Menu sync failed (non-blocking)');
    });
}

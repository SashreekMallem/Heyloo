...
export async function createOrder(payload, options = {}) {
    ...
    const { data: restaurant, error: restaurantError } = await supabase
        .from('restaurants')
        .select('id,name,tax_rate,delivery_fee,stripe_account_id,stripe_customer_id,pos_type,pos_location_id,manager_phone')
    ...
    if (parsed.paymentMethod === 'stripe_link') {
        ...
        if (stripePaymentLink) {
            const smsSent = await sendPaymentLinkSMS(parsed.customerPhone, stripePaymentLink, total, restaurant.name);
            if (smsSent) {
    ...
    const { data: insertedOrder, error: insertError } = await supabase
        ...
        .select('id,restaurant_id,customer_id,status,payment_status,subtotal,tax,delivery_fee,total,payment_method,stripe_payment_link,stripe_payment_intent_id,items,placed_at,updated_at')
        .maybeSingle();
    ...
    if (parsed.paymentMethod !== 'stripe_link') {
        ...
        if (restaurant.pos_type && restaurant.pos_type !== 'none') {
            pushOrderToPOS(parsedOrder.id, parsed.restaurantId).catch((error) => {
                logger.error({ error, orderId: parsedOrder.id }, 'Background POS push failed - will retry');
            });
        }
        else {
            // No POS integration - send SMS to manager if configured
            if (restaurant.manager_phone) {
                sendOrderNotificationSMS(restaurant.manager_phone, parsedOrder, restaurant.name).catch((error) => {
                    logger.error({ error, orderId: parsedOrder.id }, 'Failed to send order notification SMS to manager');
                });
            }
        }
    }
...
export async function attachStripePaymentResult(orderId, paymentIntentId) {

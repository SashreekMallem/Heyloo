 ...
 */
export async function pushOrderToPOS(orderId, restaurantId, retryAttempt = 0) {
    try {
        // Get order details
        ...
        if (!externalId) {
            await supabase
                ...
                .eq('id', orderId);
            await alertRestaurantStaff(restaurantId, orderId, 'POS location ID missing - please configure in settings');
        ...
        await supabase
            ...
            .eq('id', orderId);
        await alertRestaurantStaff(restaurantId, orderId, `Failed to sync order to POS after ${MAX_RETRY_ATTEMPTS} attempts: ${error.message}`);
 ...
 */
async function getDeliveryAddress(addressId) {
    if (!addressId)
        return undefined;
 ...
 */
export async function alertRestaurantStaff(restaurantId, orderId, errorMessage) {
    try {
        ...
        if (restaurant.phone_number) {
            const message = `⚠️ ${restaurant.name}: Order ${orderId.slice(-8)} requires manual entry in your POS. Reason: ${errorMessage}`;
 ...
 */
export async function retryOrderSync(orderId, restaurantId) {
    // Reset sync status
    await supabase
        .from('orders')
        .update({

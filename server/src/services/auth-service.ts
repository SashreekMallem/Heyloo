...
import type { AuthResponse, JwtPayload } from '@heyloo/shared';
...
export async function refreshTokens({ refreshToken }: RefreshInput): Promise<AuthResponse> {
  // Verify token first
  let payload;
  try {
  ...
  const newPayload: Omit<JwtPayload, 'exp'> = {
    sub: data.id,
    email: data.email,
    role: data.role,
    restaurantId: data.restaurant_id

...
export async function getPlatformOverview(
  ...
  const [usageResult, restaurantsResult] = await Promise.all([
    supabase
      ...
      .select(
        'restaurant_id,total_calls,total_minutes,total_orders,total_order_value,vapi_call_cost'
      )
  ...
  const usageRows = (usageResult.data ?? []) as SupabaseUsage[];
  const restaurantRows = (restaurantsResult.data ?? []) as SupabaseRestaurant[];
...
export async function listRestaurantSummaries(
  range: DashboardTimeRange
): Promise<RestaurantSummaryMetrics[]> {
  const { start, end } = resolveDateRange(range);
  ...
  const { data: usageData } = await supabase
    ...
    .select(
      'restaurant_id,restaurant_name,subscription_status,total_calls,total_minutes,total_orders,total_order_value'
    )
    .gte('date', start.toISOString().slice(0, 10))
    .lt('date', end.toISOString().slice(0, 10));
  ...
  return (restaurants ?? []).map((restaurant) => {
    ...
    return {
      ...
      status: (usage?.status ?? restaurant.subscription_status) as RestaurantSummaryMetrics['status'],
      calls: usage?.calls ?? 0,
...
export async function getUsageTimeline(range: DashboardTimeRange) {
  const { start, end } = resolveDateRange(range);
  ...
  const timeline = new Map<
    string,
    { totalCalls: number; totalMinutes: number; totalOrders: number; revenue: number }
  >();
...
export async function getCallCenterMetrics(range: DashboardTimeRange) {
  const { start, end } = resolveDateRange(range);

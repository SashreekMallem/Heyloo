...
export async function assertRestaurantAccess(restaurantId: string, actorRestaurantId?: string | null) {
...
export async function getRestaurantOverview(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'total_calls,total_minutes,total_orders,total_order_value,delivery_orders,pickup_orders'
    )
  ...
  if (error) {
    ...
    throw Object.assign(
      new Error('Failed to load restaurant overview metrics'),
      {
        status: 500,
        code: 'QUERY_ERROR',
        details: error
...
export async function listRestaurantOrders(restaurantId: string, limit = 50) {
  // Validate restaurantId
  if (!restaurantId || restaurantId.trim() === '' || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(restaurantId)) {
    throw Object.assign(new Error('Invalid restaurant ID'), {
      status: 400,
  ...
  const { data, error } = await supabase
    .from('orders')
    .select(
      'id,status,payment_status,total,subtotal,tax,delivery_fee,payment_method,placed_at,customer_name,customer_phone,order_type'
    )
    .eq('restaurant_id', restaurantId)
...
export async function listRecentCalls(restaurantId: string, limit = 20) {
  // Validate restaurantId
  if (!restaurantId || restaurantId.trim() === '' || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(restaurantId)) {
    throw Object.assign(new Error('Invalid restaurant ID'), {
      status: 400,
...
export async function listMenuItems(restaurantId: string, locationId?: string | null, showAll?: boolean) {
...
export async function listRestaurantCustomers(restaurantId: string) {
  const { data, error } = await supabase
    .from('customers')
    .select('*')
    .eq('restaurant_id', restaurantId)
...
export async function listCustomerAddresses(restaurantId: string, customerId: string) {
  const { data, error } = await supabase

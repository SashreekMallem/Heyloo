...
type OnboardingPayload = {
  ...
  ownerEmail: string;
  phoneNumber?: string; // Optional - will provision VAPI number if not provided
  taxRate?: number;
  deliveryFee?: number;
  ...
  posAccessToken?: string; // From OAuth flow
  posRefreshToken?: string;
  posLocationId?: string; // Required for Square, Toast
  posMerchantId?: string; // Required for Clover

  // Stripe (optional - can connect later)
...
export async function onboardRestaurant(
  ...
  if (payload.posType && payload.posType !== 'none' && payload.posAccessToken) {
    ...
    logger.warn(
      { restaurantId: restaurant.id, posType: payload.posType },
      'POS credentials provided - in production these should be encrypted'
    );
 ...
 */
export async function handlePosOAuthCallback(
  restaurantId: string,
  ...
  locationId?: string;
  merchantId?: string;
  locations?: Array<{ id: string; name: string; address?: any }>;
  merchants?: Array<{ id: string; name: string }>; // For Clover
}> {
...
async function exchangeSquareCode(code: string) {
  ...
  let locations: Array<{ id: string; name: string; address?: any }> = [];
  try {
...
async function exchangeToastCode(code: string) {
...
async function exchangeCloverCode(code: string) {
  ...
  try {
    ...
    if (merchantsResponse.ok) {
      ...
      merchants = (merchantsData.elements || []).map((merchant: any) => ({
        id: merchant.id,
 ...
 */
export async function connectMultipleLocations(
  restaurantId: string,
  posType: 'square' | 'toast' | 'clover',
  config: {
    accessToken: string;
  ...
  for (let i = 0; i < selectedIds.length; i++) {
    ...
    const { data: upsertedId, error: upsertError } = await supabase.rpc(
      'insert_restaurant_pos_location',
      {
        ...
        p_address:
          'address' in locationData && locationData.address
            ? (locationData.address as any)
            : null,
  ...
  logger.info({
    ...
    if (connectedCount > 0) {
      ...
      if (connectedLocations && connectedLocations.length > 0) {
        logger.info(
          { restaurantId, locationCount: connectedLocations.length },
          '[POS Connection] Triggering auto-menu sync for connected locations'
        );
 ...
 */
export async function updateRestaurantPosConfig(
  restaurantId: string,
  posType: 'square' | 'toast' | 'clover',
  config: {
    accessToken: string;

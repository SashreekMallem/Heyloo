import type { PosProvider } from '@heyloo/shared';
...
const singletonCache = new Map<PosProvider, any>();

export function getPosIntegration(provider: PosProvider) {
  if (provider === 'none') {
...
export function getRestaurantExternalId(provider: PosProvider, overrides?: { squareLocationId?: string }) {
  switch (provider) {
    case 'square':

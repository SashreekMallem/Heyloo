...
import type { PosMenuItem, PosOrderItem } from '@heyloo/shared';

import { env } from '../config/env.js';
import type { PosIntegration } from './base.js';

export class SquareIntegration implements PosIntegration {
  readonly provider = 'square' as const;
  private readonly defaultClient: AxiosInstance | null;
  private readonly baseURL: string;
  private readonly squareVersion = '2025-01-23';
  ...
  async pullMenu(locationId: string, accessToken?: string): Promise<PosMenuItem[]> {
    if (!locationId) {

...
onboardingRouter.get('/pos/:provider/callback', async (req, res) => {
  ...
  try {
    ...
    const tokens = await handlePosOAuthCallback(restaurantId, provider, code, state as string);
...
onboardingRouter.post('/pos/:provider/finalize', async (req, res, next) => {
  try {
    ...
    const connectedCount = await connectMultipleLocations(
      oauthSession.restaurant_id,
      provider as 'square' | 'toast' | 'clover',
      {

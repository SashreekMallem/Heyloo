...
export async function sendSMS(to: string, message: string): Promise<boolean> {
  ...
  if (!env.TWILIO_ACCOUNT_SID || !env.TWILIO_AUTH_TOKEN || !env.TWILIO_PHONE_NUMBER) {
    logger.warn(
      ...
      'Twilio not configured - SMS would be sent in production'
    );
...
export async function sendPaymentLinkSMS(
  ...
  const message = `Hi! Your order from ${restaurantName} totaling $${orderTotal.toFixed(
    2
  )} is ready. Please complete payment here: ${paymentLink}`;
...
export async function sendOrderConfirmationSMS(
  ...
  let message = `✅ Your ${orderType} order from ${restaurantName} has been confirmed! Order ID: ${orderId.slice(
    -8
  )}`;
...
export async function sendOrderNotificationSMS(
  ...
  const itemsList = order.items
    .map((item: any) => `${item.quantity}x ${item.name} ($${item.lineTotal})`)
    .join('\n');

...
export function RestaurantSettingsPage() {
  ...
  const [posAuthProvider, setPosAuthProvider] = useState<'square' | 'clover' | null>(null);
  const [posAuthSession, setPosAuthSession] = useState<string | null>(null);
  ...
  const [formData, setFormData] = useState({
    ...
    taxRate: 0,
    deliveryFee: 0,
    posType: 'none' as 'square' | 'toast' | 'clover' | 'none',
    posLocationId: '',
    // Address
    addressStreet: '',
    addressCity: '',
  ...
  useEffect(() => {
    ...
    if (posAuth && session && (posAuth === 'square' || posAuth === 'clover')) {
      ...
      setPosAuthProvider(posAuth as 'square' | 'clover');
      ...
      getPosLocations(posAuth as 'square' | 'clover', session)
        .then((result) => {
  ...
  const handlePosConnect = async (provider: 'square' | 'toast' | 'clover') => {
    try {
        ...
        if (event.data?.type === 'pos_oauth_complete') {
          ...
          if (event.data.session && event.data.provider) {
            ...
            setTimeout(() => {
              ...
              setPosAuthProvider(event.data.provider as 'square' | 'clover');
              ...
              getPosLocations(event.data.provider as 'square' | 'clover', event.data.session)
                .then((result) => {
  ...
  const handleFinalizePosConnection = async () => {
    if (!posAuthProvider || !posAuthSession) return;
    ...
    try {
      ...
      const result = await finalizePosConnection(
        posAuthProvider,
        posAuthSession,
        locationIds,
        merchantIds
  ...
  if (isLoading) {
    return (
      <div className="space-y-8 text-white">
        <div className="glass-panel p-10 text-center text-white/60">Loading settings…</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 text-white">
      <header>
        <p className="text-sm uppercase tracking-[0.3rem] text-white/50">Configuration</p>
        <h1 className="text-4xl font-bold">Restaurant Settings</h1>
        <p className="text-white/60 mt-2">
          Configure information that your AI assistant uses to answer customer questions
        </p>
      </header>
      ...
      <div className="glass-panel p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-white/70 mb-2">
              Assistant Name
              <span className="text-xs text-white/50 ml-2">(for personalized greetings)</span>
            </label>
            <input
              type="text"
              ...
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
              placeholder="e.g., Sarah, Alex, or leave empty"
            />
            <p className="text-xs text-white/50 mt-1">
              The AI will introduce itself with this name in call greetings
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-white/70 mb-2">Tax Rate (%)</label>
            <input
              type="number"
              step="0.01"
              value={formData.taxRate * 100}
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, taxRate: parseFloat(e.target.value) / 100 }))
              }
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, deliveryFee: parseFloat(e.target.value) }))
              }
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
              rows={2}
              placeholder="e.g., Free parking in front, Street parking available"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, accessibilityNotes: e.target.value }))
              }
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
              rows={2}
      ...
      <div className="glass-panel p-8 space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-4">
          <p className="text-sm text-white/70">
            These contacts will be provided to customers when the AI assistant cannot resolve their
            query
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-white/70 mb-2">Manager Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-white/70 mb-2">Manager Phone</label>
            <input
              type="tel"
              value={formData.managerPhone}
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, managerPhone: e.target.value }))
              }
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
      ...
      <div className="glass-panel p-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold">Delivery & Ordering</h2>
        </div>
        ...
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, acceptsCash: e.target.checked }))
              }
              className="rounded"
            />
            <span className="text-white">Accepts Cash</span>
          </label>
          ...
          <label className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, acceptsCard: e.target.checked }))
              }
              className="rounded"
            />
            <span className="text-white">Accepts Card</span>
          </label>
        </div>
      </div>
      ...
      <div className="glass-panel p-8 space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold">Special Instructions for AI Assistant</h2>
        </div>

        <div className="bg-white/5 border border-white/10 rounded-xl p-4">
          <p className="text-sm text-white/70">
            Provide any special instructions or information that the AI should communicate to
            customers (e.g., special offers, dietary accommodations, ordering policies)
          </p>
        </div>
        ...
        <textarea
          ...
          className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/30"
          rows={6}
          placeholder="e.g., We offer gluten-free options. Mention our lunch special from 11am-2pm. For large orders (10+ items), please call the manager directly."
        />
      </div>
      ...
      <div className="glass-panel p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-white mb-2">POS Integration</h2>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <button
                  key={pos.value}
                  type="button"
                  onClick={() => setFormData((prev) => ({ ...prev, posType: pos.value as any }))}
          ...
          {formData.posType !== 'none' ? (
            <>
              {posConfig && posConfig.lastSyncAt && (
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-white/70">Last Menu Sync</p>
                      <p className="text-sm text-white/50 mt-1">
                        {new Date(posConfig.lastSyncAt).toLocaleString()}
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/20 rounded-xl text-sm text-white hover:bg-white/20 transition-all disabled:opacity-50"
                    >
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => handlePosConnect(formData.posType as any)}
                  disabled={isConnectingPos}
                  className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl font-semibold hover:bg-white/90 transition-all disabled:opacity-50"
                >
                ...
                {/* Show disconnect button if posConfig shows connected OR if restaurants table has pos_type (for cleanup) */}
                {(() => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isSaving}
                      className="flex items-center gap-2 px-6 py-3 bg-rose-500/20 border border-rose-500/30 text-rose-300 rounded-xl font-semibold hover:bg-rose-500/30 transition-all disabled:opacity-50"
                    >
            ...
            <div className="space-y-4">
              <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                <h4 className="text-sm font-semibold text-white mb-2">Manual Menu Management</h4>
                <p className="text-sm text-white/70 mb-4">
                  Without POS integration, you can manage your menu directly. Orders will be sent to you via SMS.
                </p>
                <ul className="space-y-2 text-sm text-white/70">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-emerald-400 mt-0.5 flex-shrink-0" />
                    <span>Add/edit menu items in the Menu page</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-emerald-400 mt-0.5 flex-shrink-0" />
                    <span>New orders will be sent to your manager phone via SMS</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 text-emerald-400 mt-0.5 flex-shrink-0" />
                    <span>Update order status manually in the Orders page</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl font-semibold hover:bg-white/90 transition-all"
                >
      ...
      <div className="flex items-center justify-end gap-3 pt-6 border-t border-white/10">
        <button
          ...
          onClick={handleSave}
          disabled={isSaving}
          className="flex items-center gap-2 px-8 py-4 bg-white text-slate-900 rounded-xl font-semibold hover:bg-white/90 transition-all disabled:opacity-50 shadow-lg"
        >
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="glass-panel p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2">
                  Select {posAuthProvider === 'square' ? 'Locations' : 'Merchants'}
                </h2>
                <p className="text-white/60 text-sm">
                  Choose which {posAuthProvider === 'square' ? 'locations' : 'merchants'} you want to connect to your restaurant
                </p>
              </div>
              <button
            ...
            <div className="space-y-3 mb-6">
              {posAuthProvider === 'square' && availableLocations.map((location: any) => {
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      className="mt-1 h-4 w-4 text-white border-white/30 rounded focus:ring-2 focus:ring-white/50"
                    />
                    <div className="flex-1">
                      <div className="font-semibold text-white">{location.name}</div>
                      {location.address && (
                        <div className="text-sm text-white/60 mt-1">{location.address.address_line_1}</div>
                      )}
                      {location.capabilities && location.capabilities.length > 0 && (
                        <div className="text-xs text-white/50 mt-1">
                          Capabilities: {location.capabilities.join(', ')}
                        </div>
                      )}
                    </div>
                  </label>
              ...
              {posAuthProvider === 'clover' && availableMerchants.map((merchant: any) => {
              ...
              (posAuthProvider === 'clover' && availableMerchants.length === 0) ? (
                <div className="text-center py-8 text-white/60">
                  No {posAuthProvider === 'square' ? 'locations' : 'merchants'} available
                </div>
              ) : null}
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
              ...
              <button
                type="button"
                onClick={handleFinalizePosConnection}
                disabled={isFinalizing || (posAuthProvider === 'square' && selectedLocationIds.length === 0) || (posAuthProvider === 'clover' && selectedMerchantIds.length === 0)}
                className="px-6 py-3 bg-white text-slate-900 rounded-xl font-semibold hover:bg-white/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isFinalizing ? 'Connecting...' : `Connect ${selectedLocationIds.length + selectedMerchantIds.length} ${posAuthProvider === 'square' ? 'Location(s)' : 'Merchant(s)'}`}
              </button>
            </div>
          </div>
        </div>
